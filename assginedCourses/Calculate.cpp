...
void calculate::assignedCoursestoStudents(){
	...
	lp.obj(e);
	lp.solve();
	for (int x = 0; x < season.studentList.count(); x++)
		for (int y = 0; y < season.studentList.get(x).FinalCourses.count(); y++)
			cout << season.studentList.get(x).name.get() << "," << season.studentList.get(x).FinalCourses.get(y).name.get() << endl;

	for (int i = 0; i < m; i++){
		for (int j = 0; j < n; j++)
		{
			cout << lp.primal(matrix[i][j]);
			if (lp.primal(matrix[i][j]) == 1){
				Course c = season.courseList.get(j);
				cout << season.studentList.get(i).name.get() << "," << c.get_id() << endl;
